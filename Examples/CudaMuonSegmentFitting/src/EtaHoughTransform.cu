...
namespace HoughDetail =
    ActsExamples::CudaHoughTransformUtils::detail;

namespace PeakFinders =
    ActsExamples::CudaHoughTransformUtils::PeakFinders;

__device__ inline CoordType minimum(
    CoordType first, CoordType second) {
  return first < second ? first : second;
}

__device__ inline CoordType maximum(
    CoordType first, CoordType second) {
  return first > second ? first : second;
}

__global__ void computeEtaInterceptRangesMdtBatchKernel(
    CudaHoughPlaneBatchArrays plane,
  ...
  CoordType* sharedMaximum =
      sharedMinimum + blockDim.x;
  ...
  const std::uint32_t bucketStart =
      spacePoints.bucketStart[bucket];

  const std::uint32_t bucketEnd =
      spacePoints.bucketEnd[bucket];

  for (std::uint32_t hit =
           bucketStart + threadIdx.x;
       hit < bucketEnd;
       hit += blockDim.x) {
    const CoordType y =
        spacePoints.localPositionY[hit];

    localMinimum =
        minimum(localMinimum, y - interceptMargin);

    localMaximum =
        maximum(localMaximum, y + interceptMargin);
  }
    ...
    if (threadIdx.x < nextActive) {
      ...
      if (partner < active) {
        sharedMinimum[threadIdx.x] =
            minimum(sharedMinimum[threadIdx.x],
                    sharedMinimum[partner]);

        sharedMaximum[threadIdx.x] =
            maximum(sharedMaximum[threadIdx.x],
                    sharedMaximum[partner]);
      }
    }
  ...
  if (threadIdx.x == 0u) {
    if (bucketStart == bucketEnd ||
        sharedMinimum[0] > sharedMaximum[0]) {
...
__global__ void fillEtaDriftCirclesMdtBatchKernel(
  ...
  const std::uint32_t nCells =
      plane.nBinsX * plane.nBinsY;

  extern __shared__ unsigned char sharedMemory[];

  auto* sharedHits =
      reinterpret_cast<YieldType*>(sharedMemory);
  ...
  auto* sharedLayerMask =
      reinterpret_cast<LayerMask*>(
          sharedMemory + layerMaskOffset);

  const std::size_t candidateOffset =
      HoughDetail::alignUp(
          layerMaskOffset +
              nCells * sizeof(LayerMask),
          alignof(
              PeakFinders::GlobalMaximumCandidate));

  auto* sharedCandidates =
      reinterpret_cast<
          PeakFinders::GlobalMaximumCandidate*>(
          sharedMemory + candidateOffset);

  for (std::uint32_t bucket = blockIdx.x;
       bucket < plane.nBuckets;
       bucket += gridDim.x) {
    ...
    const std::uint32_t nTasks =
        nHits * plane.nBinsX * nSolutions;

    for (std::uint32_t task = threadIdx.x;
         task < nTasks;
         task += blockDim.x) {
      const std::uint32_t solution =
          task % nSolutions;

      const std::uint32_t xBin =
          (task / nSolutions) % plane.nBinsX;

      const std::uint32_t localHit =
          task / (nSolutions * plane.nBinsX);

      const std::uint32_t hitIndex =
          bucketStart + localHit;

      const CoordType tanTheta =
          HoughDetail::binCenterDevice(
              ranges.xMin, ranges.xMax,
              plane.nBinsX, xBin);

      const CoordType y =
          spacePoints.localPositionY[hitIndex];

      const CoordType z =
          spacePoints.localPositionZ[hitIndex];

      const CoordType radius =
          spacePoints.driftRadius[hitIndex];

      const CoordType sign =
          solution == 0u ? CoordType{-1.0}
                         : CoordType{1.0};
      ...
      const CoordType covariance =
          spacePoints.covariance1[hitIndex] > 0.0
              ? spacePoints.covariance1[hitIndex]
              : CoordType{0.0};

      CoordType width =
          sqrt(covariance) * widthScale;
      ...
      const unsigned layer =
          detLayer(spacePoints.muonId[hitIndex]);
    ...
    const std::uint32_t globalBase =
        bucket * nCells;

    for (std::uint32_t localBin = threadIdx.x;
         localBin < nCells;
         localBin += blockDim.x) {
      plane.nHits[globalBase + localBin] =
          sharedHits[localBin];

      plane.nLayers[globalBase + localBin] =
          sharedLayers[localBin];

      plane.layerMask[globalBase + localBin] =
          sharedLayerMask[localBin];
    }
...
void fillEtaDriftCirclesOnDeviceImpl(
  ...
  if (threadsPerBlock == 0u) {
    throw std::invalid_argument(
        "threadsPerBlock must be non-zero");
  }

  if (plane.nBuckets() != spacePoints.bucketCount()) {
    throw std::invalid_argument(
        "Eta Hough plane and space-point container must have the same bucket count");
  }

  if (maxima.nBuckets != plane.nBuckets()) {
    throw std::invalid_argument("Eta Hough plane and maximum batch must have the same bucket count");
  }

  if (maxima.capacityPerBucket == 0u ||
      maxima.nMaxima == nullptr) {
    throw std::invalid_argument(
        "Invalid Hough maximum device storage");
  }
  ...
  const std::size_t sharedBytes =
      HoughDetail::sharedBytesForEtaHough(
          plane.nCellsPerBucket(),
          threadsPerBlock);
  ...
  constexpr CoordType interceptMargin =
      10.0 * Acts::UnitConstants::cm;
